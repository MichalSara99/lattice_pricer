...
lattice_calibrator_ir::CalibratorIR<lattice_types::LatticeType::Binomial,TimeAxis, DeltaTime, DiscountCurve>::
	...
	const Node dt_0 = 0.01 * deltaTime[0];
	...
	for (std::size_t t = 1; t < latticeSize; ++t) {
		arrowDebreuLattice(t, 0) = (0.5 *dcf(rateLattice(t - 1, 0), deltaTime[t - 1]))*arrowDebreuLattice(t - 1, 0);
		if (t >= 2) {
			nodesSize = rateLattice.nodesAtIdx(t).size();
			for (std::size_t l = 1; l < nodesSize - 1; ++l) {

				arrowDebreuLattice(t, l) = (0.5 * dcf(rateLattice(t - 1, l), deltaTime[t - 1]))*arrowDebreuLattice(t - 1, l) +
					(0.5 * dcf(rateLattice(t - 1, l - 1), deltaTime[t - 1]))*arrowDebreuLattice(t - 1, l - 1);

			}
		}
		arrowDebreuLattice(t, t) = (0.5 * dcf(rateLattice(t - 1, t - 1), deltaTime[t - 1]))*arrowDebreuLattice(t - 1, t - 1);

		// one-dimensional optimization:
		auto objective = std::bind(calibFun, std::placeholders::_1,
			deltaTime[t - 1], DC::discount(t, discountCurve), rateLattice.nodesAtIdx(t - 1),
			arrowDebreuLattice.nodesAtIdx(t), dcf);
		auto optimizer = gsm(objective);
		
		// forward induction: 
		for (std::size_t l = 0; l < rateLattice.nodesAtIdx(t - 1).size(); ++l) {
			tuple = forwardGen(std::get<0>(optimizer), rateLattice(t - 1, l), deltaTime[t - 1], l);
...
_calibrate_impl(LatticeObject &rateLattice, Generator &&generator, DeltaTime const &deltaTime,
	...
	LatticeObject arrowDebreuLattice(rateLattice);
	// Take 10 % of the deltaTime:
	const Node dt_0 = 0.01 * deltaTime;
	// Linear interpolation between today discount factor (which is 1) and next discount factor 
	// with maturity dt_0:
	const Node d_0 = lerp(DC::discount(0, discountCurve), DC::discount(1, discountCurve), dt_0);
	// get the near-today short rate:
	...
	for (std::size_t t = 1; t < latticeSize; ++t) {
		arrowDebreuLattice(t, 0) = (0.5 * dcf(rateLattice(t - 1, 0), deltaTime))*arrowDebreuLattice(t - 1, 0);
		if (t >= 2) {
			nodesSize = rateLattice.nodesAtIdx(t).size();
			for (std::size_t l = 1; l < nodesSize - 1; ++l) {

				arrowDebreuLattice(t, l) = (0.5 * dcf(rateLattice(t - 1, l), deltaTime))*arrowDebreuLattice(t - 1, l) +
					(0.5 * dcf(rateLattice(t - 1, l - 1), deltaTime))*arrowDebreuLattice(t - 1, l - 1);

			}
		}
		arrowDebreuLattice(t, t) = (0.5 * dcf(rateLattice(t - 1, t - 1),deltaTime))*arrowDebreuLattice(t - 1, t - 1);

		// one-dimensional optimization:
		auto objective = std::bind(calibFun, std::placeholders::_1,
			deltaTime, DC::discount(t, discountCurve), rateLattice.nodesAtIdx(t-1),
			arrowDebreuLattice.nodesAtIdx(t), dcf);
		auto optimizer = gsm(objective);

		// forward induction: 
		for (std::size_t l = 0; l < rateLattice.nodesAtIdx(t - 1).size(); ++l) {
			tuple = forwardGen(std::get<0>(optimizer), rateLattice(t - 1, l), deltaTime, l);
